...
#Robuste Script (Peter's script majority re-use)
    ...
    parser.add_argument('-l', '--LipData', dest='LipidFile', required=True, help='Lipid Data filename (LipidFile.h5)')
    ...
    parser.add_argument('--maxsnr', dest='MaxSNR', default=20,
    ...
    parser.add_argument('--maxPkW', dest='MaxPeak_Width', default=80.0,
                        help='Maximum peak width (FWHM*pi) (default: %(default)s)')
    parser.add_argument('--minPkW', dest='MinPeak_Width', default=1.0,
                        help='Minimum peak width (FWHM*pi) (default: %(default)s)')
    parser.add_argument('--nBL', dest='NbBL', default=10, #default=10,
    ...
    parser.add_argument('--maxBLWatSc', dest='MaxBLandWatScaling', default=10.0,
			help='Maximum relative BL and Water to metabolite amplitudes (default: %(default))')
   ...
   # NMRFreq = float(args.NMRFreq)
    ...
    AllInSpectra, amplitudes, Metab_spectra, Lipid_BL_Wat_spectra,LipidID_spectra,BL_spectra,Water_spectra, index, SingleMetab_spectra, freq_shift, ph_shift,  peakwidth_Lor,peakwidth_Gau, snr, acqudelay,LipidScaling,BLScaling,WaterScaling = MakeTraining.MakeTrainingLipIDData(NbEx=int(args.nbex_train), MaxSNR=float(args.MaxSNR), MinSNR=float(args.MinSNR), MaxFreq_Shift=float(args.MaxFreq_Shift), MaxPeak_Width=float(args.MaxPeak_Width),MinPeak_Width=float(args.MinPeak_Width), NbBL=int(args.NbBL), MinBLWidth=float(args.MinBLWidth), MaxAcquDelay=float(args.MaxAcqDelay), MaxLipidScaling=float(args.MaxLipidScaling),MaxBLandWatScaling=float(args.MaxBLandWatScaling), LipStackFile=args.LipidFile, LipOpFile=args.LipidProjFile , verbose=args.verbose)
    ...
    with h5py.File(filename, 'w-', libver='earliest') as f:
        ...
        dset.attrs['MaxBLandWatScaling'] = float(args.MaxBLandWatScaling)
        ...
        train.create_dataset('WaterScaling', data=WaterScaling, fletcher32=True)  
    ...
    AllInSpectra, amplitudes, Metab_spectra, Lipid_BL_Wat_spectra,LipidID_spectra,BL_spectra,Water_spectra, index, SingleMetab_spectra, freq_shift, ph_shift,  peakwidth_Lor,peakwidth_Gau, snr, acqudelay,LipidScaling,BLScaling,WaterScaling = MakeTraining.MakeTrainingLipIDData( NbEx=int(args.nbex_test), MaxSNR=float(args.MaxSNR), MinSNR=float(args.MinSNR), MaxFreq_Shift=float(args.MaxFreq_Shift), MaxPeak_Width=float(args.MaxPeak_Width),MinPeak_Width=float(args.MinPeak_Width), NbBL=int(args.NbBL), MinBLWidth=float(args.MinBLWidth), MaxAcquDelay=float(args.MaxAcqDelay), MaxLipidScaling=float(args.MaxLipidScaling), MaxBLandWatScaling=float(args.MaxBLandWatScaling), LipStackFile=args.LipidFile, LipOpFile=args.LipidProjFile, verbose=args.verbose)
