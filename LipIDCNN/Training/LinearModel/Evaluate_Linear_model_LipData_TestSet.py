...
#__________________________________________________________________________________________________________________________________________-
    ...
    spectra_energy = np.sqrt(np.sum(np.abs(spectra_All)**2, axis=1))[:,None]
...
#__________________________________________________________________________________________________________________________________________-
    ...
    PredictedMetab = loaded_model.predict(In)
  
    PredictedMetab = PredictedMetab.reshape((spectra_IDMetab_stack.shape[0], spectra_IDMetab_stack.shape[1],spectra_IDMetab_stack.shape[2]))
    PredictedMetab = np.squeeze(PredictedMetab[:,:,0]+ 1j*PredictedMetab[:,:,1])

    print("PredictedMetab shape: ", PredictedMetab.shape)
    PredictedMetab *= spectra_energy
    spectra_All *= spectra_energy
    spectra_IDMetab *= spectra_energy
    spectra_Lip *= spectra_energy
    PredictedLip = spectra_All - PredictedMetab
    spectra_IDLip = spectra_All - spectra_IDMetab
