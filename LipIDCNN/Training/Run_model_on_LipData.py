...
#__________________________________________________________________________________________________________________________________________-
    ...
    with h5py.File(OutPathname, 'w', libver='earliest') as f:
        ...
        dset = LipFreeData.create_dataset('realData', data=np.real(np.transpose(Predicted_LipID,(1,0))), fletcher32=True)
        dset.attrs['Fs'] = Fs
        dset.attrs['N1'] = N1
        dset.attrs['N2'] = N2
        dset.attrs['NMRFreq'] = NMRFreq
        dset.attrs['Npt'] = Npt
        LipFreeData.create_dataset('imagData', data=np.imag(np.transpose(Predicted_LipID,(1,0))), fletcher32=True)  
